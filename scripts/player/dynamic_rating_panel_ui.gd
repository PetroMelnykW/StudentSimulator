	...
	while true:
		...
		if (result != null):
			...
			for index in range(panel_count):	
				var student_data = result[index] if index < len(result) else null
				
				_update_student_panel(
					_student_panels.get_child(index), 
					student_data
				)
		else:
...
func _update_student_panel(panel: Panel, student_data: Variant) -> void:
	if student_data == null:
		...
		var nameLabel = panel_h_box.get_node("PlayerNameLabel");
		var scoreLabel = panel_h_box.get_node("PlayerScoreLabel");

		nameLabel.text = student_data.nickname + ":"
		scoreLabel.text = str(student_data.score)
